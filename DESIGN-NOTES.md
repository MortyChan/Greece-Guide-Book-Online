...
- Use the V4 motion tokens: `--motion-press`, `--motion-fast`, `--motion-base`, `--motion-swap`, `--motion-scene`, `--ease-enter`, and `--ease-exit`.
...
- Keep the Dock hidden while the hero is substantially visible; reveal it once handbook content becomes the primary viewport.
